...
class SSD(nn.Module):
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k=0):
        ...
        predicted_scores = F.softmax(predicted_scores, dim=2)  # (N, 1940, n_classes)
        ...
        for i in range(batch_size):
            ...
            decoded_locs = ssd_utils.cxcy_to_xy(
                ssd_utils.gcxgcy_to_cxcy(predicted_locs[i], self.priors_cxcy))  # (1940, 4), these are fractional pt. coordinates
            ...
            max_scores, best_label = predicted_scores[i].max(dim=1)  # (1940)

            # Check for each class
            for c in range(1, self.config.NUM_CLASSES):
                # Keep only predicted boxes and scores where scores for this class are above the minimum score
                class_scores = predicted_scores[i][:, c]  # (1940)
                score_above_min_score = class_scores > min_score  # torch.uint8 (byte) tensor, for indexing
                n_above_min_score = score_above_min_score.sum().item()
                if n_above_min_score == 0:
                    continue
                class_scores = class_scores[score_above_min_score]  # (n_qualified), n_min_score <= 1940
                ...
                overlap = ssd_utils.find_jaccard_overlap(class_decoded_locs, class_decoded_locs)  # (n_qualified, n_min_score)
...
class MultiBoxLoss(nn.Module):
    def __init__(self, priors_cxcy, config : SSDConfig):
        ...
        self.priors_xy = ssd_utils.cxcy_to_xy(priors_cxcy).to(self.config.DEVICE)
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        :param predicted_locs: predicted locations/boxes w.r.t the 1940 prior boxes, a tensor of dimensions (N, 1940, 4)
        :param predicted_scores: class scores for each of the encoded locations/boxes, a tensor of dimensions (N, 1940, n_classes)
        ...
        true_locs = torch.zeros((batch_size, n_priors, 4), dtype=torch.float).to(self.config.DEVICE)  # (N, 1940, 4)
        true_classes = torch.zeros((batch_size, n_priors), dtype=torch.long).to(self.config.DEVICE)  # (N, 1940)

        # For each image
        for i in range(batch_size):
            # n_objects = boxes[i].size(0)

            overlap = ssd_utils.find_jaccard_overlap(boxes[i], self.priors_xy)  # (n_objects, 1940)

            # For each prior, find the object that has the maximum overlap
            overlap_for_each_prior, object_for_each_prior = overlap.max(dim=0)  # (1940)
            # print(overlap_for_each_prior)
            # Labels for each prior
            label_for_each_prior = labels[i][object_for_each_prior]  # (1940)
            # Set priors whose overlaps with objects are less than the threshold to be background (no object)
            label_for_each_prior[overlap_for_each_prior < self.threshold] = 0  # (1940)

            # Store
            true_classes[i] = label_for_each_prior

            # Encode center-size object coordinates into the form we regressed predicted boxes to
            true_locs[i] = ssd_utils.cxcy_to_gcxgcy(ssd_utils.xy_to_cxcy(boxes[i][object_for_each_prior]), self.priors_cxcy)  # (1940, 4)

        # Identify priors that are positive (object/non-background)
        positive_priors = true_classes != 0  # (N, 1940)

        # LOCALIZATION LOSS
        # print(positive_priors)
        # Localization loss is computed only over positive (non-background) priors
        loc_loss = self.smooth_l1(predicted_locs[positive_priors], true_locs[positive_priors])  # (), scalar
        # Note: indexing with a torch.uint8 (byte) tensor flattens the tensor when indexing is across multiple dimensions (N & 1940)
        # So, if predicted_locs has the shape (N, 1940, 4), predicted_locs[positive_priors] will have (total positives, 4)
        ...
        # print(f"Positive priors: {n_positives}")
        n_hard_negatives = self.neg_pos_ratio * n_positives  # (N)

        # First, find the loss for all priors
        conf_loss_all = self.cross_entropy(predicted_scores.view(-1, n_classes), true_classes.view(-1))  # (N * 1940)
        conf_loss_all = conf_loss_all.view(batch_size, n_priors)  # (N, 1940)
        # We already know which priors are positive
        conf_loss_pos = conf_loss_all[positive_priors]  # (sum(n_positives))
        # print(f'conf_loss pos: {conf_loss_pos}')

        # Next, find which priors are hard-negative
        # To do this, sort ONLY negative priors in each image in order of decreasing loss and take top n_hard_negatives
        conf_loss_neg = conf_loss_all.clone()  # (N, 1940)
        conf_loss_neg[positive_priors] = 0.  # (N, 1940), positive priors are ignored (never in top n_hard_negatives)
        conf_loss_neg, _ = conf_loss_neg.sort(dim=1, descending=True)  # (N, 1940), sorted by decreasing hardness
        
        hardness_ranks = torch.LongTensor(range(n_priors)).unsqueeze(0).expand_as(conf_loss_neg).to(self.config.DEVICE)  # (N, 1940)
        hard_negatives = hardness_ranks < n_hard_negatives.unsqueeze(1)  # (N, 1940)
